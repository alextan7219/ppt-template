...
Private Sub DownloadLatestTemplate(ByVal LocalPath As String, ByVal URL As String)
    Dim needsDownload As Boolean
    needsDownload = True

    ' If a cached copy already exists and is less than 24 hours old, reuse it
    ' instead of hitting GitHub again. This is the main fix for the 429
    ' "Too Many Requests" error - it cuts request volume drastically since
    ' the template is only re-downloaded once per day at most.
    If Dir(LocalPath) <> "" Then
        If (Now - FileDateTime(LocalPath)) < 1 Then ' 1 day = 24 hours
            needsDownload = False
        End If
    End If

    If Not needsDownload Then
        Debug.Print "Using cached template (downloaded within last 24h)"
        Exit Sub
    End If
    ...
    ' A proper User-Agent reduces the chance GitHub's abuse detection
    ' flags the request as scraping and throttles it.
    http.setRequestHeader "User-Agent", "SolarProposalMacro/1.0"
    http.send
    ...
    ElseIf http.Status = 429 Then
        ' Rate limited by GitHub. Fall back to the cached copy if one
        ' exists instead of hard-failing the whole macro.
        If Dir(LocalPath) <> "" Then
            Debug.Print "Rate limited by GitHub (429). Using existing cached copy instead."
        Else
            MsgBox "GitHub is rate-limiting downloads (429 Too Many Requests) and " & _
                   "no cached template exists yet on this machine." & vbCrLf & vbCrLf & _
                   "Please wait a few minutes and try again.", vbExclamation
            Err.Raise vbObjectError + 1, , "Download failed - rate limited (429)"
        End If
...
Sub GeneratePowerPointFromTemplate_v1()
    ...
    templatePath = Environ("TEMP") & "\TP_Template_ATAP_Small_below_8.04.pptx"   ' Temporary local copy
    
    Dim templateURL As String
    templateURL = "https://raw.githubusercontent.com/alextan7219/ppt-template/main/TP%20Template_ATAP_Small_below_8.04.pptx"
    ...
    fileNameBase = "Solar_Proposal_" & ws.Range("C1").Text & "_" & ws.Range("C2").Text & "_" & ws.Range("C4").Text & "_" & ws.Range("C6").Text & "_" & ws.Range("C10").Text
    ...
    ' Slide 17 replacements
    Dim slide17 As Object
    Set slide17 = pptPres.Slides(17)
   
    For Each shp In slide17.Shapes
        Select Case shp.Name
            Case "panel_size"
                shp.TextFrame.TextRange.Text = ws.Range("C27").Text
            Case "inverter"
    ...
    MsgBox "PowerPoint generated successfully!" & vbCrLf & _
           ...
           "Template was up to date (downloaded fresh or reused from cache)."
End Sub
